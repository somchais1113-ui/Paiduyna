...
function isRun(activity) {
  const value = String(activity.sport_type || activity.type || "");
  return ["Run", "TrailRun", "VirtualRun"].includes(value) || String(activity.type || "") === "Run";
}
...
export async function latestRunToday() {
  ...
  const latest = Array.isArray(activities)
    ? activities.filter(isRun).sort((a, b) => new Date(b.start_date) - new Date(a.start_date))[0]
    : null;
  ...
  return {
    ...
    activity: {
      id: String(detail.id),
      name: detail.name || "Morning Run",
      sport_type: detail.sport_type || detail.type || "Run",
